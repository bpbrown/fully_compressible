...
grad_θ0_g = de.Grid(grad(θ0)).evaluate()
...
grad_θ = grad(θ1) + ez*lift1(τ_s2,-1)
...
problem.add_equation((ρ0*(ddt(u)
                      ...
                      - ρ0_g*(u@grad(u))
                      # grad(h) terms
                      - ρ0_grad_h0_g*(np.expm1(θ1)-θ1)
                      - ρ0_h0_g*np.expm1(θ1)*grad(θ1)
                      # h*grad(s) terms
                      + ρ0_h0_grad_s0_g*(np.expm1(θ1)-θ1)
                      + ρ0_h0_g*np.expm1(θ1)*grad(s1)
...
# boundary conditions
problem.add_equation((θ1(z=0), 0))
problem.add_equation((θ1(z=Lz), 0))
if no_slip:
