...
grad_h0_g = de.Grid(grad_h0).evaluate()
...
problem.add_equation((h0*(ddt(Υ) + trace(grad_u) + u@grad_Υ0),
                      -h0_g*(u@grad(Υ)) ))
problem.add_equation((h0*ρ0*(ddt(s) + u@grad_s0)
                      - h0*scrP*(div(grad_θ) + 2*grad_θ0@grad_θ)
                      + lift(τ_s1,-1),
                      - ρ0_h0_g*(u@grad(s))
