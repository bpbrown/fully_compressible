...
grad_T = grad(T1) + ez*lift1(τ_s2,-1)
...
for ncc in [ρ0, ρ0*grad_T0, ρ0*T0, ρ0*grad_Υ0, h0*grad_Υ0]:
...
s1_pert = noise['g']*np.sin(np.pi*z/Lz)
θ1['g'] = s1_pert
Υ1['g'] = -θ1['g']
T1.change_scales(dealias)
T1['g'] = (T0*np.expm1(θ1)).evaluate()['g']
#not quite right, off by gamma?
...
s1 = cV/cP*np.log(1+T1/T0)-(1/cP)*Υ1
h1 = cP*T1
...
T = h/cP
KE = 0.5*ρ*u@u
IE = 1/γ*ρ*h # cV/cP*ρ*h = cV*ρ*T = ρ*e
φ = grad_φ*z_grid
PE = ρ*φ
...
while solver.proceed and good_solution:
    ...
    if solver.iteration % report_cadence == 0:
        ...
        log_string = 'Iteration: {:5d}, Time: {:8.3e} ({:.1e}), dt: {:5.1e}'.format(solver.iteration, solver.sim_time, solver.sim_time*μ, Δt)
