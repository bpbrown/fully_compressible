...
τ_frame = d.VectorField(c, name='τ_frame')
...
problem.add_equation((ρ0*(dt(u) + 1/Ma2*(h0*grad(θ) + grad_h0*θ)
                      ...
                      - R_inv*viscous_terms
                      + lift(τ_u1,-1) + lift(τ_u2,-2) + τ_frame,
                      -ρ0_g*dot(u,grad(u))
...
problem.add_equation((integ(dot(ex,ρ0*u)), 0)) #not perfect, but a start; should be full rho
